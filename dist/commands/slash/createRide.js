...
export const data = new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName('vibe')
    .setDescription('The vibe of the ride')
    .setRequired(true)
    .addChoices({ name: '🌶️ Spicy', value: 'SPICY' }, { name: '🎉 Party', value: 'PARTY' }))
    .addStringOption(option => option.setName('type')
    .setDescription('The type of ride')
    .setRequired(true)
    .addChoices({ name: 'Road', value: 'ROAD' }, { name: 'Gravel', value: 'GRAVEL' }, { name: 'Mountain', value: 'MOUNTAIN' }, { name: 'Social', value: 'SOCIAL' }, { name: 'Virtual', value: 'VIRTUAL' }))
    .addStringOption(option => option.setName('drop_style')
    .setDescription('The drop style of the ride')
    .setRequired(true)
    .addChoices({ name: 'Drop', value: 'DROP' }, { name: 'No Drop', value: 'NO_DROP' }, { name: 'Regroup', value: 'REGROUP' }))
    .addStringOption(option => option.setName('date')
    .setDescription('The date of the ride (e.g., "tomorrow", "next Tuesday", "MM/DD")')
    .setRequired(true))
    .addStringOption(option => option.setName('meet_time')
    .setDescription('When to arrive (e.g., "9:00 AM", "21:00")')
    .setRequired(true))
    .addStringOption(option => option.setName('rollout_time')
    .setDescription('Minutes after meet time to start')
    .setRequired(true)
    .addChoices({ name: '0 minutes', value: '0' }, { name: '15 minutes', value: '15' }, { name: '30 minutes', value: '30' }, { name: '45 minutes', value: '45' }, { name: '60 minutes', value: '60' }))
    .addStringOption(option => option.setName('start_location')
    .setDescription('Where to meet')
    .setRequired(true)
    .addChoices({ name: 'Angry Catfish', value: 'ANGRY_CATFISH' }, { name: 'Northern Coffeeworks', value: 'NORTHERN_COFFEEWORKS' }, { name: 'Other', value: 'OTHER' }))
    .addStringOption(option => option.setName('end_location')
    .setDescription('Where the ride ends (defaults to start location)')
    .setRequired(false)
    .addChoices({ name: 'Same as start', value: 'SAME' }, { name: 'Angry Catfish', value: 'ANGRY_CATFISH' }, { name: 'Northern Coffeeworks', value: 'NORTHERN_COFFEEWORKS' }, { name: 'Other', value: 'OTHER' }))
    .addStringOption(option => option.setName('distance')
    .setDescription('Distance in miles or kilometers (e.g., "20 miles" or "32 km")')
    .setRequired(false))
    .addStringOption(option => option.setName('route_source')
...
export async function execute(interaction) {
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: 'Date', value: formatDate(date), inline: true }, { name: 'Meet', value: `${meetTime} - Rollout: ${rolloutTime}`, inline: true }, { name: 'Start', value: formatLocation(startLocation), inline: true }, { name: 'Drop Style', value: dropStyle, inline: true });
